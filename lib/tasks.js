...
import { generateDailyIntro } from './daily-intro.js';
import { pickNextTask } from './task-picker.js';
...
// Uses the smart picker (Haiku-driven) when memory has signal; otherwise
// falls back to strict sequential via getNextTaskForUser.
export async function deliverTaskToUser(user) {
  ...
  let pickReason = null;
  try {
    const picked = await pickNextTask(user);
    task = picked?.task || null;
    pickReason = picked?.reason || null;
  } catch (err) {
    console.error('[deliverTaskToUser] picker failed, falling back:', err.message);
  }
  if (!task) task = await getNextTaskForUser(user);
  ...
  // Generate a personalized one-line intro from the brain. If it fails or
  // memory is empty we still deliver the task — never block on intro.
  const intro = await generateDailyIntro(user, task).catch(() => null);

  if (intro) {
    await sendMessage(user.telegram_id, `_${intro}_`);
  }
  ...
  await logActivity(user.id, 'task_delivered', {
    task_id: task.id,
    personalized: !!intro,
    pick_reason: pickReason,
  });
  return { ok: true, task_id: task.id, intro, pick_reason: pickReason };
}
