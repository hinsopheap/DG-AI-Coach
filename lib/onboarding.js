      ...
      try {
        ...
        if (path) {
          ...
          try {
            ...
            const latest = await getUserByTelegramId(fresh.telegram_id);
            if (latest) await checkOnboardingAchievement(latest);
          } catch {}
          await sendMessage(
            chatId,
            `✅ You are enrolled in *${path.title}*.\n\nUse /today for today's task, /topics to pick a starter, /web to open the web chat, or just chat freely.`,
            removeKeyboard(),
          );
          // Pickup any pending topic stashed during /start topic_<id>
          if (fresh.pending_topic_id) {
            try {
              const { TOPICS, topicPrompt, noteTopicInterest } = await import('./topics.js');
              const topic = TOPICS.find(t => t.id === fresh.pending_topic_id);
              if (topic) {
                await updateUser(fresh.id, { pending_topic_id: null });
                noteTopicInterest(fresh.id, topic.id).catch(() => {});
                const lang = fresh.preferred_language === 'km' ? 'km' : 'en';
                await sendMessage(chatId, `Picking up the topic you started with: *${topic.label_en}*`);
                // Defer to coach via a regular freeform message
                const { coachTurn } = await import('./coach.js');
                const result = await coachTurn({ user: { ...fresh, status: 'active' }, surface: 'telegram', text: topicPrompt(topic, lang) });
                if (result?.reply) await sendMessage(chatId, result.reply);
              }
            } catch (e) { console.error('[onboarding] pending_topic pickup failed:', e); }
          }
        } else {
